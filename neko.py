...
async def commands(client, message):
    ...
    if text.startswith("/start"):
        ...
        await message.reply(f"Buscando imágenes de {hcode}")
        h3_in_use = True
        ...
        for link in image_links:
            try:
                ...
                await message.reply(f"Error al descargar la imagen: {str(e)}")
                h3_in_use = False
                return

        await message.reply(f"Creando un CBZ")
        ...
        await message.reply(f"Las imágenes de {hcode} fueron enviadas en un CBZ correctamente")
        h3_in_use = False
        ...
        if replied_message:
            ...
            await message.reply("Descargando...")
            file = await client.download_media(replied_message.document.file_id)
